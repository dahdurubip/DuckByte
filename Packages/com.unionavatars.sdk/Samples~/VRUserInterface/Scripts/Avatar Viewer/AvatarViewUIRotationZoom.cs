...
namespace UnionAvatars.VRUI
{
    public class AvatarViewUIRotationZoom : MonoBehaviour, IDragHandler, IEndDragHandler, IBeginDragHandler
    {
        ...
        const float maxFov = 45;
        const float avatarParentLowPosition = -1.352f;
        const float avatarParentHighosition = -0.817f;
        ...
        public void OnDrag(PointerEventData eventData)
        {
            ...
            newParentYPosition += mouseAxisY * (ZoomSpeed / 50f); //Fov/Position rate
            newParentYPosition = Mathf.Clamp(newParentYPosition, avatarParentLowPosition, avatarParentHighosition);
            AvatarParent.localPosition = new Vector3(AvatarParent.localPosition.x, newParentYPosition, AvatarParent.localPosition.z);
        }
