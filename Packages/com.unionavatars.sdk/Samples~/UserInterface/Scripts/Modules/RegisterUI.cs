...
namespace UnionAvatars.UI
{
    public class RegisterUI : UIModule
    {
        ...
        [SerializeField]
        private GameObject verificationUI;

        [SerializeField]
        private TextMeshProUGUI verificationEmailText;
        ...
        [SerializeField]
        private TextMeshProUGUI usernameWarning;

        [SerializeField]
        private TextMeshProUGUI emailWarning;

        [SerializeField]
        private TextMeshProUGUI passwordWarning;

        [SerializeField]
        private TextMeshProUGUI confirmPasswordWarning;
        ...
        public async Task<bool> CheckUsername()
        {
            ...
            WebResponse<string> userExistsResponse = await WebRequests.Send<string>(
                uiManager.session.SessionContext.Url + "users/exists/username/" + usernameField.text,
                "GET",
                uiManager.session.SessionContext
            );
        ...
        public async Task<bool> CheckEmail()
        {
            ...
            {
                emailWarning.text = "Invalid format";
                emailWarning.gameObject.SetActive(true);
                return false;
            }

            WebResponse<string> userExistsResponse = await WebRequests.Send<string>(
                uiManager.session.SessionContext.Url + "users/exists/email/" + emailField.text,
                "GET",
                uiManager.session.SessionContext
            );
        ...
        {
            if (!await CheckUsername() || !await CheckEmail() || !CheckPassword() || !CheckConfirmationPassword())
                return;
            ...
            //ADD Editor check and proper error handling, show docs
